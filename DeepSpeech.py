...
tf.app.flags.DEFINE_string  ('train_files',      'data/multiple_20.csv',          'comma separated list of files specifying the dataset used for training. multiple files will get merged')
tf.app.flags.DEFINE_string  ('dev_files',        'data/test_vystadial.csv',          'comma separated list of files specifying the dataset used for validation. multiple files will get merged')
tf.app.flags.DEFINE_string  ('test_files',       'data/test_vystadial.csv',          'comma separated list of files specifying the dataset used for testing. multiple files will get merged')
...
tf.app.flags.DEFINE_integer ('epoch',            50,          'target epoch to train - if negative, the absolute number of additional epochs will be trained')

tf.app.flags.DEFINE_boolean ('use_warpctc',      False,       'wether to use GPU bound Warp-CTC')

tf.app.flags.DEFINE_float   ('dropout_rate',     0.3,        'dropout rate for feedforward layers')
...
tf.app.flags.DEFINE_integer ('dev_batch_size',   12,           'number of elements in a validation batch')
tf.app.flags.DEFINE_integer ('test_batch_size',  12,           'number of elements in a test batch')
...
tf.app.flags.DEFINE_integer ('display_step',     1,           'number of epochs we cycle through before displaying detailed progress - 0 means no progress display')
tf.app.flags.DEFINE_integer ('validation_step',  5,           'number of epochs we cycle through before validating the model - a detailed progress report is dependent on "--display_step" - 0 means no validation steps')

# Checkpointing

tf.app.flags.DEFINE_string  ('checkpoint_dir',   'ckpt/'+ckpt_folder,          'directory in which checkpoints are stored - defaults to directory "deepspeech/checkpoints" within user\'s data home specified by the XDG Base Directory Specification')
tf.app.flags.DEFINE_integer ('checkpoint_secs',  300,         'checkpoint saving interval in seconds')
tf.app.flags.DEFINE_integer ('max_to_keep',      1,           'number of checkpoint files to keep - default value is 5')
...
tf.app.flags.DEFINE_integer ('n_hidden',         128,        'layer width to use when initialising layers')
...
tf.app.flags.DEFINE_string  ('decoder_library_path', 'native_client2/libctc_decoder_with_kenlm.so', 'path to the libctc_decoder_with_kenlm.so library containing the decoder implementation.')
...
def BiRNN(batch_x, seq_length, dropout):
    ...
    lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(n_cell_dim, forget_bias=1.0, state_is_tuple=True) \
                   ...
                   tf.nn.rnn_cell.BasicLSTMCell(n_cell_dim, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
...
def decode_with_lm(inputs, sequence_length, beam_width=100,
    ...
    return ([tf.SparseTensor(ix, val, shape) for (ix, val, shape)
            in zip(decoded_ixs, decoded_vals, decoded_shapes)], log_probabilities)
...
def collect_results(results_tuple, returns):
    ...
    for i in range(1):  # len(available_devices)):
        # Collect the labels
...
class Epoch(object):
    ...
    Kwargs:
        ...
        report (bool): if this job should produce a WER report
    '''
    def __init__(self, index, num_jobs, set_name='train', report=True):
        self.id = new_id()
        self.index = index
